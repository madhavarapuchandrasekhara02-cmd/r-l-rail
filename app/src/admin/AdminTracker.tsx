...
export default function AdminTracker() {
  ...
  // Calculate active steps for 8-node timeline
  ...
  const step4Active = 
    ['shipped', 'delivered'].includes(ordStatus) && (
      ['picked up', 'dispatched', 'in transit', 'reached hub', 'out for delivery', 'delivered'].includes(liveStatus) ||
      ['shipped', 'delivered'].includes(dbStatus) ||
      scanStatuses.length > 0
    )
  const step5Active = 
    ['in transit', 'reached hub', 'out for delivery', 'delivered'].includes(liveStatus) ||
    ['in transit', 'delivered'].includes(dbStatus) ||
    scanStatuses.some((s: string) => s.includes('transit') || s.includes('shipped') || s.includes('dispatched'))
  const step6Active = 
    ['reached hub', 'out for delivery', 'delivered'].includes(liveStatus) ||
    scanStatuses.some((s: string) => s.includes('hub') || s.includes('sorting') || s.includes('arrived')) ||
    scanLocations.some((l: string) => l.includes('hub') || l.includes('sorting') || l.includes('facility'))
  const step7Active = 
    ['out for delivery', 'delivered'].includes(liveStatus) ||
    scanStatuses.some((s: string) => s.includes('out for delivery') || s.includes('outfordelivery'))
  const step8Active = ordStatus === 'delivered' || liveStatus === 'delivered' || dbStatus === 'delivered'
  ...
  const steps = [
    ...
    { status: 'Picked Up', icon: Truck, active: step4Active, desc: 'Parcel collected by courier' },
    { status: 'In Transit', icon: MapPin, active: step5Active, desc: 'Traveling to sorting hubs' },
    { status: 'Reached Hub', icon: Building, active: step6Active, desc: 'Arrived at local city depot' },
    { status: 'Out For Delivery', icon: Clock, active: step7Active, desc: 'Courier out for delivery today' },
    { status: 'Delivered', icon: CheckCircle2, active: step8Active, desc: 'Successfully handed over' }
  ]
                      ...
                      {steps.map((step, i) => (
                        <div key={i} className="relative flex items-start gap-4">
                          ...
                          <div className="flex flex-col mt-0.5">
                            <h4 className={`text-[10px] font-bold uppercase tracking-widest ${step.active ? 'text-[#4A3525]' : 'text-[#B37943]/30'}`}>
                              {step.status}
                            </h4>
                            <span className="text-[9px] text-[#B37943] italic mt-0.5 leading-tight">{step.desc}</span>
                          </div>
                  ...
                  <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#FAF3E8]">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] font-bold text-[#B37943] uppercase tracking-wider mt-0.5">AWB Tracking ID: {waybill}</div>
                      </div>
                    </div>

                    <a
                      href={`https://track.delhivery.com/track/package/${waybill}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="h-10 px-4 bg-white hover:bg-[#FAF9F6] border border-[#E5C492] text-[#4A3525] rounded-xl text-[10px] font-sans font-bold uppercase tracking-widest transition-all flex items-center justify-center cursor-pointer shadow-sm"
                    >
                      Open Live Portal
                    </a>
                  </header>
